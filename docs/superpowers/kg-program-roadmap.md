...
| KG-3 | **Infra / deployment modeling** | Closed the other gap. Shipped as a **derived** overlay (ADR-0029: `Service`/`EnvVar` nodes + `requires`/`runs`/`talks_to`/`configured_by` edges from `docker-compose.yml`), new `tools/infra/` domain + `docs/infra/` catalog. | deployment `gap` scenarios: KG-1 passed by *reporting* no topology → KG-3 re-run **traverses** the service DAG, entrypoints, backing stores + config; the service-topology agent even reasoned read-vs-write backing services (RESULTS.md "KG-3 re-run") | ✅ **DONE** |

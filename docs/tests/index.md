...
- `agents.test_anthropic_agent` (7) → agents  ·  verifies: —
- `agents.test_claude_code_agent` (2) → agents  ·  verifies: —
- `agents.test_failover_agent` (5) → agents  ·  verifies: —
- `agents.test_openai_agent` (5) → agents  ·  verifies: —
- `agents.test_structured_outputs` (0) → agents  ·  verifies: —
- `api.test_analysis_rewire` (3) → api  ·  verifies: —
- `api.test_ask_router` (5) → api  ·  verifies: —
- `api.test_edit_api_unit` (8) → api  ·  verifies: —
...
- `api.test_ui_router` (22) → api  ·  verifies: —
...
- `ask.test_engine` (1) → ask  ·  verifies: —
- `ask.test_fusion` (5) → ask  ·  verifies: —
- `ask.test_reader` (3) → ask  ·  verifies: —
...
- `commands.test_command_handlers` (0) → commands  ·  verifies: —
- `commands.test_command_handlers_unit` (2) → commands  ·  verifies: —
- `enrichment.test_embedder` (4) → enrichment  ·  verifies: —
- `enrichment.test_executor` (1) → enrichment  ·  verifies: —
- `enrichment.test_final_review_fixes` (2) → enrichment  ·  verifies: —
- `enrichment.test_graph_context` (5) → enrichment  ·  verifies: —
- `enrichment.test_orchestrator` (0) → enrichment  ·  verifies: —
- `enrichment.test_orchestrator_document_pass` (0) → enrichment  ·  verifies: —
...
- `events.test_core_plumbing_validation` (16) → events  ·  verifies: —
...
- `events.test_repository_unit` (11) → events  ·  verifies: —
...
- `events.test_store_unit` (7) → events  ·  verifies: —
- `export.test_bundler` (0) → export  ·  verifies: —
- `export.test_reader` (0) → export  ·  verifies: —
...
- `ingestion.test_front_matter_seeding` (1) → ingestion  ·  verifies: —
- `ingestion.test_golden_transcript` (0) → ingestion  ·  verifies: —
- `ingestion.test_models` (3) → ingestion  ·  verifies: —
- `ingestion.test_normalizer` (6) → ingestion  ·  verifies: —
- `ingestion.test_orchestrator` (0) → ingestion  ·  verifies: —
- `ingestion.test_sample_corpus` (8) → ingestion  ·  verifies: —
- `ingestion.test_speaker_inference` (1) → ingestion  ·  verifies: —
- `ingestion.test_stitcher` (1) → ingestion  ·  verifies: —
- `knowledge.test_check` (6) → tools.knowledge  ·  verifies: —
- `knowledge.test_cli` (5) → tools.knowledge  ·  verifies: —
- `lens.test_engine` (0) → lens  ·  verifies: —
...
- `persistence.test_graph_persistence` (3) → persistence  ·  verifies: —
- `projections.test_bootstrap_unit` (12) → projections  ·  verifies: —
- `projections.test_config_unit` (2) → projections  ·  verifies: —
- `projections.test_embedding_handlers` (1) → projections  ·  verifies: —
- `projections.test_entity_claim_handlers` (1) → projections  ·  verifies: —
- `projections.test_health_unit` (29) → projections  ·  verifies: —
- `projections.test_lane_manager_unit` (0) → projections  ·  verifies: —
- `projections.test_lens_handlers` (1) → projections  ·  verifies: —
- `projections.test_metrics_unit` (35) → projections  ·  verifies: —
- `projections.test_migrate_shim_drop` (0) → projections  ·  verifies: —
- `projections.test_parked_events_unit` (6) → projections  ·  verifies: —
- `projections.test_projection_handlers_unit` (3) → projections  ·  verifies: —
- `projections.test_projection_service_unit` (15) → projections  ·  verifies: —
- `projections.test_redrive` (2) → projections  ·  verifies: —
- `projections.test_registry_unit` (16) → projections  ·  verifies: —
- `projections.test_reorder_buffer` (13) → projections  ·  verifies: —
- `projections.test_resolution_handlers` (0) → projections  ·  verifies: —
- `projections.test_run_projection_service_unit` (6) → projections  ·  verifies: —
- `projections.test_schema` (3) → projections  ·  verifies: —
- `projections.test_segment_handlers` (0) → projections  ·  verifies: —
- `projections.test_speaker_handlers` (2) → projections  ·  verifies: —
- `projections.test_subscription_manager_unit` (9) → projections  ·  verifies: —
- `projections.test_utterance_handlers` (1) → projections  ·  verifies: —
...
- `resolution.test_engine` (0) → resolution  ·  verifies: —
- `resolution.test_reader` (0) → resolution  ·  verifies: —
- `resolution.test_suggestions` (0) → resolution  ·  verifies: —
- `test_anthropic_agent_response` (3) → —  ·  verifies: —
- `test_celery_app` (15) → —  ·  verifies: —
- `test_config` (25) → —  ·  verifies: —
- `test_openai_agent_response` (3) → —  ·  verifies: —
- `test_prompts` (10) → —  ·  verifies: —
...
- `ui.test_notifications` (12) → ui  ·  verifies: —
- `ui.test_reader` (0) → ui  ·  verifies: —
...
- `utils.test_logging` (0) → utils  ·  verifies: —
- `utils.test_metrics_performance` (8) → utils  ·  verifies: —
- `utils.test_metrics_tracker_comprehensive` (21) → utils  ·  verifies: —
- `utils.test_neo4j_driver` (0) → utils  ·  verifies: —
- `utils.test_path_helpers` (4) → utils  ·  verifies: —
- `utils.test_text_processing` (7) → utils  ·  verifies: —

## integration

- `integration.test_anthropic_api_messages` (0) → —  ·  verifies: —
- `integration.test_api_calls` (0) → —  ·  verifies: —
- `integration.test_idempotency` (0) → —  ·  verifies: —
- `integration.test_migrate_shim_drop_live` (0) → —  ·  verifies: —
- `integration.test_multi_provider_api_live` (0) → —  ·  verifies: —
- `integration.test_neo4j_connection_reliability` (6) → —  ·  verifies: —
- `integration.test_neo4j_data_integrity` (0) → —  ·  verifies: —
- `integration.test_openai_api_responses` (0) → —  ·  verifies: —
- `integration.test_projection_replay` (0) → —  ·  verifies: —
- `integration.test_prompt_validation_live` (0) → —  ·  verifies: —

## e2e

- `integration.test_ask_smoke` (0) → —  ·  verifies: —
- `integration.test_deployed_projection_smoke` (0) → —  ·  verifies: —
- `integration.test_e2e_user_edits` (0) → —  ·  verifies: use-cases:correct-what-the-system-got-wrong
- `integration.test_end_to_end_smoke` (0) → —  ·  verifies: use-cases:surface-the-signal
- `integration.test_layer1_projection_smoke` (0) → —  ·  verifies: —
- `integration.test_layer2_enrichment_smoke` (0) → —  ·  verifies: —
- `integration.test_layer3_lens_smoke` (0) → —  ·  verifies: —
- `integration.test_layer4_resolution_smoke` (0) → —  ·  verifies: —
- `integration.test_layer5_export_smoke` (0) → —  ·  verifies: —
- `integration.test_live_feed_smoke` (0) → —  ·  verifies: —
- `integration.test_projection_ordering_smoke` (0) → —  ·  verifies: —

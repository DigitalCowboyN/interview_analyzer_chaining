...
MOCK_CONFIG = {
    "paths": {"output_dir": MOCK_OUTPUT_DIR, "analysis_suffix": MOCK_ANALYSIS_SUFFIX}
}
...
def test_list_analysis_files_empty():
    ...
    with patch.dict("src.config.config", MOCK_CONFIG, clear=True), patch(
        "src.api.routers.files.Path"
    ) as MockPath:  # Patch constructor

        # Configure the mock instance returned for Path(MOCK_OUTPUT_DIR)
        mock_dir_path_instance = MagicMock(spec=Path)
        MockPath.return_value = (
            mock_dir_path_instance  # Simplest case: assume only Path(dir) is called
        )
...
def test_list_files_internal_error():
    ...
    with patch.dict("src.config.config", MOCK_CONFIG, clear=True), patch(
        ...
        assert "Internal server error listing analysis files" in response.json().get(
            "detail", ""
        )
...
SAMPLE_JSONL_CONTENT = (
    f"{json.dumps(SAMPLE_CONTENT_LINE_1)}\n{json.dumps(SAMPLE_CONTENT_LINE_2)}\n"
)
...
def test_get_file_content_malformed_json():
    ...
    with patch.dict("src.config.config", MOCK_CONFIG, clear=True), patch(
        "src.api.routers.files.Path"
    ) as MockPath, patch("src.api.routers.files.logger") as mock_logger:
        ...
        assert any(
            "Skipping malformed JSON line" in call.args[0]
            and "this is not json" in call.args[0]
...
def test_get_specific_sentence_success():
    ...
    line1 = {
        ...
        "sequence_order": 1,
        "sentence": "Second target.",
        "analysis": "B",
        "topic_level_1": "Target Topic",
    }
    line2 = {
        "sentence_id": 2,
        "sequence_order": 2,

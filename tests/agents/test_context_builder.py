...
def test_init_success(mock_config_valid):
    ...
    assert (
        builder.context_windows == mock_config_valid["preprocessing"]["context_windows"]
    )
...
def test_init_missing_windows(mock_config_missing_windows, caplog):
    ...
    # Ensure the old error log is NOT present
    assert (
        "Config key missing for context_windows: 'context_windows'" not in caplog.text
    )
...
def test_build_context_large_window(sample_sentences, mock_config_valid):
    ...
    context = builder.build_context(
        sample_sentences, idx=2, window_size=10
    )  # Window > list size
    expected = (
        "Sentence 0.\n"
        "Sentence 1.\n"
...
def test_build_context_invalid_index(sample_sentences, mock_config_valid):
    ...
    assert (
        builder.build_context(
            sample_sentences, idx=len(sample_sentences), window_size=1
        )
        == ""
    )
...
def test_build_all_contexts_success(sample_sentences, mock_config_valid):
    ...
    assert len(all_contexts) == len(
        sample_sentences
    )  # Should have entry for each sentence
    ...
    expected_narrow_s0 = ">>> TARGET: Sentence 0. <<<\n" "Sentence 1."

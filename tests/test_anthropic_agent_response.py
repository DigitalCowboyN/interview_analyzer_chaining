"""
tests/test_anthropic_agent_response.py

Comprehensive tests for the AnthropicAgent that follow cardinal rules:
...
class TestAnthropicAgentInitialization:
    """Test AnthropicAgent initialization with realistic configurations."""

    def test_initialization_with_valid_config(self):
        """Test agent initializes correctly with valid Anthropic configuration."""
        # Mock config to avoid requiring actual API key
        test_config = {
            "anthropic": {
                "api_key": "test-key-sk-ant-1234567890abcdef",
                "model_name": "claude-3-haiku-20240307",
                "max_tokens": 1000,
                "temperature": 0.7,
            },
            "anthropic_api": {"retry": {"max_attempts": 3, "backoff_factor": 2}},
        }

        with patch("src.agents.anthropic_agent.config", test_config):
    ...
    def test_initialization_fails_with_missing_api_key(self):
        ...
        invalid_config = {"anthropic": {"api_key": "", "model_name": "claude-3-haiku-20240307"}}  # Empty API key should fail

        with patch("src.agents.anthropic_agent.config", invalid_config):
    ...
    def test_initialization_with_default_retry_settings(self):
        ...
        minimal_config = {
            "anthropic": {"api_key": "test-key", "model_name": "claude-3-haiku-20240307", "max_tokens": 500, "temperature": 0.5}
            # No anthropic_api section - should use defaults
        }

        with patch("src.agents.anthropic_agent.config", minimal_config):
            agent = AnthropicAgent()
...
class TestAnthropicAgentAPIInteraction:
    ...
    def configured_agent(self):
        ...
        test_config = {
            "anthropic": {
                "api_key": "test-key-sk-ant-realistic",
                "model_name": "claude-3-haiku-20240307",
                "max_tokens": 1000,
                "temperature": 0.3,
            },
            "anthropic_api": {"retry": {"max_attempts": 2, "backoff_factor": 1.5}},  # Reduced for faster tests
        }

        with patch("src.agents.anthropic_agent.config", test_config):
            return AnthropicAgent()

    def create_realistic_anthropic_response(self, content_dict: Dict[str, Any]) -> MagicMock:
        """
        Create a realistic Anthropic API response structure.

        This mirrors the actual structure returned by the Anthropic Messages API,
        matching what the agent expects: response.content[0].text
        """
        # Create the response structure that matches anthropic.messages.create
        mock_response = MagicMock()
        mock_content = MagicMock()

        # The agent expects response.content[0].text as JSON string
        mock_content.text = json.dumps(content_dict)
        mock_response.content = [mock_content]

        # Add realistic metadata
        mock_response.id = "msg-realistic-test-id"
    ...
    async def test_successful_api_call_with_realistic_interview_prompt(self, configured_agent):
        ...
        with patch.object(agent.client.messages, "create", new_callable=AsyncMock) as mock_create:
            mock_create.return_value = self.create_realistic_anthropic_response(expected_response)
    ...
    async def test_json_parsing_with_realistic_malformed_responses(self, configured_agent):
        ...
        mock_response_invalid_json.content = [mock_content]
    ...
    async def test_retry_logic_with_transient_errors(self, configured_agent):
        ...
        with patch.object(agent.client.messages, "create", new_callable=AsyncMock) as mock_create:
            ...
            mock_create.side_effect = [
                ...
                self.create_realistic_anthropic_response(successful_response),
            ]
...
class TestAnthropicAgentIntegration:
    ...
    def interview_analysis_agent(self):
        ...
        with patch("src.agents.anthropic_agent.config", interview_config):
            return AnthropicAgent()

    def create_realistic_anthropic_response(self, content_dict: Dict[str, Any]) -> MagicMock:
        """Helper method to create realistic Anthropic response structure."""
        mock_response = MagicMock()
        mock_content = MagicMock()
        mock_content.text = json.dumps(content_dict)
        mock_response.content = [mock_content]
        ...
        # Add realistic metadata
        mock_response.id = "msg-integration-test"
        mock_response.model = "claude-3-haiku-20240307"
    ...
    async def test_concurrent_api_calls_simulation(self, interview_analysis_agent):
        ...
        with patch.object(agent.client.messages, "create", new_callable=AsyncMock) as mock_create:
            mock_create.side_effect = [self.create_realistic_anthropic_response(resp) for resp in expected_responses]
    ...
    async def test_prompt_formatting_and_response_processing(self, interview_analysis_agent):
        ...
        with patch.object(agent.client.messages, "create", new_callable=AsyncMock) as mock_create:
            mock_create.return_value = self.create_realistic_anthropic_response(realistic_analysis)

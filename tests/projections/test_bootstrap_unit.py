...
class TestCreateHandlerRegistry:
    ...
    def test_create_handler_registry_registers_all_handlers(
        ...
        """Registry should have exactly 17 handlers registered (7 core + 5 speaker + 3 utterance + 2 enrichment)."""
        mock_get_client.return_value = MagicMock()

        registry = create_handler_registry()

        registered_types = registry.get_registered_types()
        assert len(registered_types) == 17
    ...
    def test_create_handler_registry_logs_initialization_message(
        ...
        assert "17 handlers" in caplog.text

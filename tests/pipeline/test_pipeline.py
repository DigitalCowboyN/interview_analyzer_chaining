...
def create_mock_analysis(
    sentence_id: int, sequence_order: int, sentence_text: str
) -> Dict[str, Any]:
    """
...
def mock_config(tmp_path: Path) -> Dict[str, Any]:
    ...
    return {
        ...
        "classification": {
            "local": {"prompt_files": {"no_context": "mock/path/to/prompts.yaml"}}
        },
...
def mock_analysis_service() -> MagicMock:
    ...
    mock.context_builder.build_all_contexts = MagicMock(
        return_value={
            0: {"ctx": "all_ctx_0"},
            1: {"ctx": "all_ctx_1"},
            2: {"ctx": "all_ctx_2"},
    ...
    mock.analyze_sentences = AsyncMock(
        return_value=[{"result": "mock"}]
    )  # Async method
...
async def test_analyze_specific_sentences_success(
    tmp_path: Path, mock_analysis_service: MagicMock
) -> None:
    ...
    target_contexts_sorted: List[Dict[str, str]] = [
        {"ctx": "target_ctx_1"},
        {"ctx": "target_ctx_3"},
    ]
    ...
    mock_analysis_service.analyze_sentences = AsyncMock(
        return_value=mock_service_results
    )

    # Patch the internal helpers
    with patch(
        "src.pipeline._prepare_data_for_specific_analysis", new_callable=AsyncMock
    ) as mock_prepare, patch(
        "src.pipeline._build_contexts_for_specific_analysis"
    ) as mock_build_ctx, patch(
        "src.pipeline._post_process_specific_results"
        ...
        mock_prepare.return_value = (
            target_sentences_sorted,
            target_indices_sorted,
            all_sentences_text,
        )
        mock_build_ctx.return_value = target_contexts_sorted
        mock_post_process.return_value = (
            expected_final_results  # Simulate post-processing
        )
        ...
        mock_prepare.assert_awaited_once_with(
            mock_map_storage, sentence_ids_to_analyze, f"[Task {task_id}] "
        )
        ...
        mock_post_process.assert_called_once_with(
            mock_service_results, target_indices_sorted, f"[Task {task_id}] "
        )
        ...
        assert any(
            "Starting analysis for specific sentences" in log_call
            for log_call in info_calls
        ), "Pipeline should log analysis start"
        assert any(
            "Finished specific sentence analysis" in log_call for log_call in info_calls
...
async def test_analyze_specific_sentences_prepare_error(
    ...
    with patch(
        "src.pipeline._prepare_data_for_specific_analysis",
...
@pytest.mark.asyncio
async def test_run_pipeline_instantiates_and_executes_orchestrator(
    ...
    with patch(
        "src.pipeline.PipelineOrchestrator", autospec=True
    ) as MockPipelineOrchestrator:
        ...
        mock_orchestrator_instance.execute.assert_awaited_once_with(
            specific_file=specific_file
        )


@pytest.mark.asyncio
async def test_run_pipeline_handles_orchestrator_init_error(
    mock_config: Dict[str, Any]
) -> None:
    ...
    with patch("src.pipeline.PipelineOrchestrator", side_effect=init_error), patch(
        "src.pipeline.logger"
    ) as mock_logger:
...
@pytest.mark.asyncio
async def test_pipeline_orchestrator_build_contexts_returns_list(
    ...
    orchestrator = PipelineOrchestrator(
        input_dir=input_dir, config_dict=mock_config, task_id="test-build-contexts"
    )
    ...
    assert len(contexts) == len(
        sentences
    ), f"Expected {len(sentences)} contexts, got {len(contexts)}"

    # Verify each context is a dict with expected keys
    for i, context in enumerate(contexts):
        assert isinstance(
            context, dict
        ), f"Context {i} should be dict, got {type(context)}"
        # Check for expected context keys (based on real AnalysisService output)
        expected_keys = {"immediate", "broader", "observer"}
        assert all(
            key in context for key in expected_keys
        ), f"Context {i} missing expected keys: {context}"
...
async def test_pipeline_orchestrator_analyze_sentences_accepts_list_contexts(
    ...
    orchestrator = PipelineOrchestrator(
        input_dir=input_dir, config_dict=mock_config, task_id="test-analyze-contexts"
    )
...
async def test_pipeline_orchestrator_type_compatibility_integration(
    ...
    orchestrator = PipelineOrchestrator(
        input_dir=input_dir, config_dict=mock_config, task_id="test-integration"
    )
...
def test_pipeline_orchestrator_context_format_consistency(
    ...
    orchestrator = PipelineOrchestrator(
        input_dir=input_dir, config_dict=mock_config, task_id="test-consistency"
    )
    ...
    for i, context in enumerate(contexts):
        ...
        if i == 0:
            ...
            assert (
                set(context.keys()) == expected_keys
            ), f"Context {i} has different keys: {context.keys()}"

    # Verify the context structure matches what the analysis service expects
    # (This tests the actual integration point)
    assert all(
        isinstance(ctx, dict) for ctx in contexts
    ), "All contexts should be dicts"
...
def test_pipeline_orchestrator_wrong_type_annotations_would_fail(
    ...
    orchestrator = PipelineOrchestrator(
        input_dir=input_dir, config_dict=mock_config, task_id="test-wrong-types"
    )
...
async def test_pipeline_orchestrator_text_data_source_integration(
    ...
    orchestrator = PipelineOrchestrator(
        input_dir=input_dir, config_dict=mock_config, task_id="test-text-source"
    )

    # Test the _setup_file_io method which creates TextDataSource
    data_source, map_storage, analysis_writer, paths = orchestrator._setup_file_io(
        test_file
    )
    ...
    ), "Should create TextDataSource instance"
    assert data_source.get_identifier() == str(
        test_file
    ), "Identifier should match file path"
    ...
    num_sentences, sentences = await orchestrator._read_and_segment_sentences(
        data_source
    )

...
class TestPipelineNeo4jPerformance:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_file_processing_performance(
        self,
        clean_test_database,
        tmp_path,
        performance_config,
    ):
        """Test pipeline performance with multiple files processed concurrently."""
        import time
        ...
        for file_idx in range(num_files):
            sentences = []
            for sent_idx in range(sentences_per_file):
                sentences.append(f"File {file_idx} sentence {sent_idx} for concurrent processing test.")

            content = " ".join(sentences)
            test_file = input_dir / f"concurrent_file_{file_idx:03d}.txt"
            test_file.write_text(content)
        ...
        with patch(
            ...
            # Create orchestrator
            class ConcurrentOrchestrator(PipelineOrchestrator):
                def __init__(self, *args, **kwargs):
                    self.project_id = project_id
                    self.interview_id = interview_id
                    super().__init__(*args, **kwargs)
            ...
            orchestrator = ConcurrentOrchestrator(
                ...
                map_dir=map_dir,
                config_dict=performance_config,
                task_id="concurrent_performance_test",
            )
        ...
        total_sentences = num_files * sentences_per_file
        print(f"Concurrent processing time: {total_time:.2f}s for {num_files} files ({total_sentences} sentences)")
        ...
        assert total_time / num_files < 60.0  # Should average less than 1 minute per file

        # Verify all files were processed
        async with await Neo4jConnectionManager.get_session() as session:
            result = await session.run("MATCH (s:Sentence) " "RETURN DISTINCT s.filename as filename")
            filenames = []
            async for record in result:
                filenames.append(record["filename"])

            assert len(filenames) == num_files  # Should have all files

            # Verify sentence counts
            total_stored_sentences = 0
            for filename in filenames:
                ...
                count = await result.single()
                total_stored_sentences += count["count"]

            assert total_stored_sentences == total_sentences  # Should have all sentences

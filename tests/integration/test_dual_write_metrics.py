...
class TestDualWriteMetrics:
    ...
    @pytest.mark.skip(reason="M2.8: No more direct Neo4j writes - projection service handles all writes")
    async def test_metrics_track_neo4j_failure_after_event_success(
        self,
        clean_test_database,
        clean_event_store,
    ):
        """
        [SKIPPED - M2.8] Test that metrics correctly track Neo4j failures after successful event emission.
        ...
        - No neo4j_after_event metrics to track

        Original scenario:
        1. Reset metrics
        2. Mock Neo4j to fail
        3. Write sentence (event succeeds, Neo4j fails)
        4. Verify metrics:
           - event_emission_success incremented
           - dual_write_event_first_success incremented
           - dual_write_neo4j_after_event_failure incremented

        This validated:
        - Metrics distinguish event success from Neo4j failure
        - Both success and failure metrics can increment in same operation
        """
        # === STEP 1: Reset metrics ===
        metrics_tracker.reset()
        ...
        # Use random interview ID to avoid stream conflicts
        ...
        with patch.object(
            Neo4jConnectionManager, "get_session"
        ) as mock_get_session:
            ...
            # === STEP 3: Write entry (event succeeds, Neo4j fails) ===
            entry = {
                "sentence_id": 0,
                "sequence_order": 0,
                "sentence": "Test sentence",
            ...
            # Should NOT raise (event succeeded, Neo4j failure is non-critical)
            await storage.write_entry(entry)
        ...
        sentence_created_success = events_metrics.get("emission_success_SentenceCreated", 0)
        assert sentence_created_success >= 1, f"Expected >=1 SentenceCreated success, got {sentence_created_success}"

        # Event-first success
        event_first_success = dual_write_metrics.get("event_first_success", 0)
        assert event_first_success >= 1, f"Expected >=1 event_first_success, got {event_first_success}"

        # Neo4j after event FAILURE
        neo4j_after_event_failure = dual_write_metrics.get("neo4j_after_event_failure", 0)
        assert (
            neo4j_after_event_failure >= 1
        ), f"Expected >=1 neo4j_after_event_failure, got {neo4j_after_event_failure}"

        # Neo4j after event SUCCESS should be 0
        neo4j_after_event_success = dual_write_metrics.get("neo4j_after_event_success", 0)
        assert neo4j_after_event_success == 0, f"Expected 0 neo4j_after_event_success, got {neo4j_after_event_success}"

        print("\n✓ Neo4j failure metrics validation passed:")
        print(f"  - SentenceCreated success: {sentence_created_success}")
        print(f"  - Event-first success: {event_first_success}")
        print(f"  - Neo4j after event failure: {neo4j_after_event_failure}")
        print("  - Event succeeded, Neo4j failed (correct dual-write behavior)")

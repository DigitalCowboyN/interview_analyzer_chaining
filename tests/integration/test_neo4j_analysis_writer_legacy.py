...
"""
Legacy integration tests for Neo4j analysis writer using direct-write path.

**DEPRECATED in M2.8**: These tests validate the direct Neo4j write path (without event_emitter).
This path is deprecated in M2.8 event-sourced architecture where events are the source of truth.

**Status**: Kept for backward compatibility reference. Tests are skipped in CI.

**Rationale**:
- M2.8 uses event-first dual-write pattern (events → projections → Neo4j)
- These tests validate the legacy fallback path when event_emitter=None
- Useful for ensuring backward compatibility during transition
- Consider removing entirely in M3.0 when direct-write path is removed

**Migrating to M2.8 Pattern**:
To migrate a test from this file to M2.8 pattern:
1. Add clean_event_store fixture
2. Create PipelineEventEmitter with event store
3. Pass event_emitter to Neo4jAnalysisWriter/Neo4jMapStorage
4. Call process_events_through_projection() after write_result()
5. Use uuid5-based sentence IDs for verification queries

**Recommended Tests to Migrate**:
- test_zero_cardinality_limit (cardinality enforcement)
- test_multi_dimension_edit_protection (edit protection + multi-value)
- test_mixed_edit_protection (edit protection + mixed dimensions)
- test_edit_protection_with_cardinality_limits (edit protection + cardinality)
- test_project_*_limit_override (8 tests for project limits)
"""
...
# Mark all tests in this module as asyncio, integration, and SKIPPED
pytestmark = [
    pytest.mark.asyncio,
    pytest.mark.integration,
    pytest.mark.skip(
        reason="Legacy direct-write path - deprecated in M2.8 event-sourced architecture. "
               "See test_neo4j_analysis_writer.py for M2.8 event-first tests."
    ),
]


# --- Tests for Cardinality Limits (Direct Write) ---
...
# --- Tests for Edit Flag Protection (Direct Write) ---
...
# --- Tests for Error Result Handling (Direct Write) ---
...
# --- Tests for Database Error Handling (Mock-Based) ---
...
# --- Tests for Project Cardinality Overrides (Direct Write) ---

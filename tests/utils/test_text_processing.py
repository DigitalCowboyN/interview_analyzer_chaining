...
VALID_TEXT_CASES = [
    ...
    ("Sentence one? Sentence two! Sentence three...", ["Sentence one?", "Sentence two!", "Sentence three..."]),
    ("Mr. Smith went to Washington D.C. on Jan. 1st.", ["Mr. Smith went to Washington D.C. on Jan. 1st."]), # Test abbreviations
    (" This sentence has leading/trailing spaces. ", ["This sentence has leading/trailing spaces."]),
...
def test_segment_text_spacy_load_failure():
    ...
    with patch.object(text_processing, 'nlp', None):
        ...
        with patch.object(text_processing, 'logger') as mock_logger:
            ...
            mock_logger.error.assert_called_once_with("spaCy model not loaded. Cannot segment text.")
...
def test_spacy_model_loading_success(mock_logger, mock_spacy_load):
    ...
    if text_processing.nlp is None:
         pytest.skip("Test assumes spaCy model loaded successfully in test environment setup.")
...
@patch("src.utils.text_processing.spacy.load", side_effect=OSError("Mock loading error"))
@patch("src.utils.text_processing.logger")
def test_spacy_model_loading_failure(mock_logger, mock_spacy_load):
    ...
    with patch.object(text_processing, 'nlp', None):
        ...
        pytest.skip("Testing import-time exception logging is difficult without reloading. Skipping direct log check.") 
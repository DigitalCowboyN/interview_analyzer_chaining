...
def generate_pipeline_paths(
    ...
    if not input_file.name or not input_file.stem:
        logger.error(f"{prefix}Input file path '{input_file}' lacks a valid filename/stem for generating output filenames.")
        raise ValueError(f"Input file path '{input_file}' must have a valid filename stem.")

    map_file_path = map_dir / f"{input_file.stem}{map_suffix}"
    analysis_file_path = output_dir / f"{input_file.stem}{analysis_suffix}"

    logger.debug(f"{prefix}Generated paths: Map='{map_file_path}', Analysis='{analysis_file_path}'")

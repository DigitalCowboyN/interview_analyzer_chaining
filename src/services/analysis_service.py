...
from src.agents.context_builder import (
    ContextBuilder,  # Assuming ContextBuilder class exists or using duck typing
)
...
from src.agents.sentence_analyzer import (
    SentenceAnalyzer,  # Import SentenceAnalyzer CLASS
)
from src.utils.logger import get_logger
from src.utils.metrics import (
    MetricsTracker,  # Assuming MetricsTracker class exists or using duck typing
)
...
class AnalysisService:
    ...
    async def analyze_sentences(
        ...
        timer: Callable[
            [], float
        ] = time.monotonic,  # Add timer dependency with default
    ) -> List[Dict[str, Any]]:
        ...
        if len(sentences) != len(contexts):
            logger.error(
                f"Sentence count ({len(sentences)}) and context count ({len(contexts)}) "
                f"mismatch in analyze_sentences. Aborting."
            )
            return []

        num_analysis_workers = self.config.get("pipeline", {}).get(
            "num_analysis_workers", 1
        )
        ...
        for i in range(num_analysis_workers):
            ...
            task = asyncio.create_task(
                self._analysis_worker(
                    i, self.analyzer, task_queue, results_queue, timer=timer
                )
            )
        ...
        loader_task = asyncio.create_task(
            self._load_tasks_from_memory(sentences, contexts, task_queue)
        )
        ...
        try:
            # Sort, handle potential missing key
            analysis_results.sort(key=lambda x: x.get("sequence_order", float("inf")))
        except TypeError as e:
            logger.error(
                f"Could not sort analysis results by sequence_order: {e}. Results might be out of order."
            )
    ...
    async def _load_tasks_from_memory(
        self,
        sentences: List[str],
        contexts: List[Dict[str, str]],
        ...
        try:
            for idx, sentence_text in enumerate(sentences):
                if idx < len(contexts):
                    ...
                    logger.warning(
                        f"Context missing for sentence index {idx}. Skipping loading task."
                    )
            ...
            logger.error(
                f"Unexpected error loading tasks from memory: {e}", exc_info=True
            )
            # self.metrics_tracker.increment_errors() # TODO (Metrics): Reinstate with proper Celery solution
            raise
    ...
    async def _analysis_worker(
        ...
        while True:
            ...
            try:
                ...
                start_time = timer()
                analysis_result: Dict[str, Any] = await analyzer.classify_sentence(
                    sentence, context
                )
                end_time = timer()

                # Use the injected metrics_tracker instance
                self.metrics_tracker.add_processing_time(sentence_id, end_time - start_time)  # type: ignore
                self.metrics_tracker.increment_sentences_success()  # Increment success metric  # type: ignore
                ...
                logger.debug(
                    f"Worker {worker_id} successfully analyzed sentence_id: {sentence_id}"
                )

            except Exception as e:
                logger.error(
                    f"Worker {worker_id} failed analyzing sentence_id {sentence_id}: {e}",
                    exc_info=True,
                )
                # Use the injected metrics_tracker instance
                self.metrics_tracker.increment_errors()  # Increment error metric  # type: ignore
                ...
                # Depends on if start_time was set

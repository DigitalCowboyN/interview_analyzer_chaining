...
from .protocols import ConversationMapStorage, SentenceAnalysisWriter, TextDataSource
...
class LocalTextDataSource(TextDataSource):
    ...
    async def read_text(self) -> str:
        ...
        try:
            ...
            logger.error(
                f"OS error reading local text file {self._file_path}: {e}",
                exc_info=True,
            )
            raise
        except Exception as e:
            logger.error(
                f"Unexpected error reading local text file {self._file_path}: {e}",
                exc_info=True,
            )
            raise
...
class LocalJsonlAnalysisWriter(SentenceAnalysisWriter):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self._file_handle = await aiofiles.open(
                self._file_path, mode="a", encoding="utf-8"
            )
            logger.debug(f"Initialized local JSONL writer for: {self._file_path}")
        except OSError as e:
            logger.error(
                f"Failed to initialize/open local analysis writer {self._file_path}: {e}",
                exc_info=True,
            )
            raise
    ...
    async def write_result(self, result: Dict[str, Any]) -> None:
        ...
        if self._file_handle is None:
            ...
            logger.error(
                f"Attempted to write to uninitialized writer: {self.get_identifier()}"
            )
        ...
        try:
            ...
            logger.error(
                f"Error writing analysis result to {self.get_identifier()}: {e}",
                exc_info=True,
            )
    ...
    async def finalize(self) -> None:
        ...
        if self._file_handle:
            try:
                ...
                logger.error(
                    f"Error closing analysis writer file {self.get_identifier()}: {e}",
                    exc_info=True,
                )
                ...
                async for line in f:
                    ...
                    try:
                        ...
                        if "sentence_id" in entry:
                            ...
                            logger.warning(
                                f"Missing 'sentence_id' in analysis file "
                                f"{self.get_identifier()}, line content: {line[:50]}..."
                            )
                    except json.JSONDecodeError:
                        logger.error(
                            f"Failed to parse line in analysis file {self.get_identifier()}: '{line[:100]}...'",
                            exc_info=True,
                        )
        except FileNotFoundError:
            logger.warning(
                f"Analysis file not found when reading IDs: {self.get_identifier()}"
            )
            # Return empty set if file doesn't exist yet (or was deleted)
        except Exception as e:
            logger.error(
                f"Unexpected error reading analysis IDs from {self.get_identifier()}: {e}",
                exc_info=True,
            )
...
class LocalJsonlMapStorage(ConversationMapStorage):
    ...
    def __init__(self, file_path: Path):
        if not isinstance(file_path, Path):
            raise TypeError("file_path must be a Path object")
        self._file_path = file_path
        self._write_handle: Optional[aiofiles.threadpool.text.AsyncTextIOWrapper] = None
    ...
    async def initialize(self) -> None:
        ...
        try:
            self._file_path.parent.mkdir(parents=True, exist_ok=True)
            self._write_handle = await aiofiles.open(
                self._file_path, mode="w", encoding="utf-8"
            )
            logger.debug(
                f"Initialized local JSONL map storage for writing: {self._file_path}"
            )
        except OSError as e:
            logger.error(
                f"Failed to initialize/open local map storage {self._file_path}: {e}",
                exc_info=True,
            )
            raise
    ...
    async def write_entry(self, entry: Dict[str, Any]) -> None:
        ...
        if self._write_handle is None:
            logger.error(
                f"Attempted to write map entry to uninitialized storage: {self.get_identifier()}"
            )
        ...
        try:
            ...
            logger.error(
                f"Error writing map entry to {self.get_identifier()}: {e}",
                exc_info=True,
            )
            raise
    ...
    async def finalize(self) -> None:
        ...
        if self._write_handle:
            try:
                ...
                logger.debug(
                    f"Finalized local JSONL map storage writing: {self._file_path}"
                )
                self._write_handle = None
            except Exception as e:
                logger.error(
                    f"Error closing map storage file {self.get_identifier()}: {e}",
                    exc_info=True,
                )
                raise
            ...
            async with aiofiles.open(self._file_path, mode="r", encoding="utf-8") as f:
                async for line in f:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to parse line in map file {self.get_identifier()}: '{line[:100]}...'",
                            exc_info=True,
                        )
        except FileNotFoundError:
            logger.warning(
                f"Map file not found when reading entries: {self.get_identifier()}"
            )
            # Return empty list as per original behavior assumption
        except Exception as e:
            logger.error(
                f"Unexpected error reading map entries from {self.get_identifier()}: {e}",
                exc_info=True,
            )
        ...
        try:
            async with aiofiles.open(self._file_path, mode="r", encoding="utf-8") as f:
                async for line in f:
                    ...
                    try:
                        entry = json.loads(line)
                        if "sentence_id" in entry and isinstance(
                            entry["sentence_id"], int
                        ):
                            ids.add(entry["sentence_id"])
                        else:
                            logger.warning(
                                f"Missing or invalid 'sentence_id' in map file "
                                f"{self.get_identifier()}, line content: {line[:50]}..."
                            )
                    except json.JSONDecodeError:
                        logger.error(
            ...
            logger.warning(
                f"Map file not found when reading IDs: {self.get_identifier()}"
            )
        except Exception as e:
            logger.error(
                f"Unexpected error reading map IDs from {self.get_identifier()}: {e}",
                exc_info=True,
            )

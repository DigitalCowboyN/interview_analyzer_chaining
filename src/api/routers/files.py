...
# Import the response schema and new models
from src.api.schemas import AnalysisResult, FileContentResponse, FileListResponse
...
def get_output_dir() -> Path:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Server configuration error for paths."
        )
...
def get_analysis_suffix() -> str:
    ...
    try:
        ...
        if not isinstance(suffix, str):
            ...
            raise HTTPException(
                status_code=500, detail="Server configuration error for suffix type."
            )
        return suffix
        ...
        logger.critical(
            f"Error retrieving analysis_suffix from config: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail="Server configuration error accessing suffix."
        )
...
@router.get(
    "/", response_model=FileListResponse, summary="List Available Analysis Files"
)
async def list_analysis_files():
    ...
    try:
        ...
        analysis_suffix = config.get("paths", {}).get(
        ...
        analysis_files = [
            f.name for f in output_dir.glob(f"*{analysis_suffix}") if f.is_file()
        ]
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error listing analysis files."
        )
...
async def get_analysis_file_content(filename: str):
    ...
    try:
        ...
        if not file_path.is_file():
            ...
            raise HTTPException(
                status_code=404, detail=f"Analysis file not found: {filename}"
            )
        ...
        try:
                ...
                for line in f:
                    ...
                    if line:
                        try:
                            ...
                            logger.warning(
                                f"Skipping malformed JSON line in {filename}: {line[:100]}..."
                            )
                            # Decide whether to skip or raise 500 - skipping for now
                        except (
                            Exception
                        ) as pydantic_err:  # Catch potential Pydantic validation error
                            logger.warning(
                                f"Skipping line due to validation/data error in {filename}: "
                                f"{pydantic_err}. Line: {line[:100]}..."
                            )
            ...
            raise HTTPException(
                status_code=500, detail=f"Error reading file: {filename}"
            )
        ...
        logger.error(
            f"Unexpected error retrieving content for file {filename}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500, detail="Internal server error retrieving file content."
        )
...
async def get_specific_sentence_analysis(filename: str, sentence_id: int):
    ...
    try:
        ...
        if not file_path.is_file():
            logger.warning(
                f"Analysis file not found for specific sentence request: {file_path}"
            )
            raise HTTPException(
                status_code=404, detail=f"Analysis file not found: {filename}"
            )
        ...
        try:
                ...
                for line_num, line in enumerate(f):
                    ...
                    try:
                        ...
                        if data.get("sentence_id") == sentence_id:
                            logger.info(
                                f"Found sentence {sentence_id} in {filename} at line {line_num + 1}"
                            )
                            ...
                            try:
                                ...
                                logger.error(
                                    f"Data validation failed for sentence {sentence_id} in {filename}: "
                                    f"{validation_err}. Data: {data}"
                                )
                                # Raise 500 as this indicates bad data was written for the specific requested sentence
                                raise HTTPException(
                                    status_code=500,
                                    detail=f"Data validation error for sentence {sentence_id}",
                                )
                    except json.JSONDecodeError:
                        logger.warning(
                            f"Skipping malformed JSON line {line_num + 1} in {filename} "
                            f"while searching for sentence {sentence_id}"
                        )
                        continue  # Keep searching
                    # Catch the specific HTTPException from validation and re-raise it
                    except HTTPException as http_exc:
                        raise http_exc
                    # Catch other unexpected errors during line processing
                    # (excluding JSONDecodeError and ValidationError handled above)
                    except Exception as inner_e:
                        logger.warning(
                            f"Skipping line {line_num + 1} in {filename} due to unexpected error "
                            f"processing line data (excluding JSON/Validation): {inner_e}. "
                            f"Line: {line[:100]}..."
                        )
                        continue  # Keep searching
            ...
            logger.error(
                f"Error reading analysis file {filename} while searching for sentence {sentence_id}: {e}",
                exc_info=True,
            )
            raise HTTPException(
                status_code=500, detail=f"Error reading file: {filename}"
        ...
        if found_result:
            ...
            raise HTTPException(
                status_code=404,
                detail=f"Sentence ID {sentence_id} not found in file {filename}",
            )
        ...
        logger.error(
            f"Unexpected error retrieving sentence {sentence_id} for file {filename}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
            detail="Internal server error retrieving sentence analysis.",
        )
